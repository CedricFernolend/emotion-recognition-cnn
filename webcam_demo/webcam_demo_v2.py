...
3. Spatial Attention - Raw attention maps from model (V4 only)

Controls:
- '1-3': Switch visualization mode
- 'g': Toggle grayscale input mode
- 'm': Switch between V4 and V1 models
...
class MultiVisualizer:
    ...
    def _register_hooks(self):
        ...
        # Target the last conv layer in layer4 (V4) or block3 (V1) for GradCAM
        ...
        # Capture spatial attention outputs (V4 only)
        if self.model_version == 'v3':
    ...
    def get_spatial_attention(self, input_tensor):
        """Get combined spatial attention maps (V4 only)."""
        if self.model_version != 'v3':
...
class EmotionDemo:
    ...
    def __init__(self, camera_id=0):
        ...
        print("    '3' - Spatial Attention (V4 only)")
        print("    'g' - Toggle grayscale input mode")
        print("    'm' - Switch model (V4 <-> V1)")
    ...
    def _load_current_model(self):
        ...
        model_path = os.path.join(RESULTS_PATH, self.current_version, 'models', 'emotion_model.pth')
        print(f"\nLoading {self.current_version.upper()} model from: {model_path}")

        if self.current_version == 'v3':
            from models.v4_final import EmotionCNN
        else:  # v1
            from models.v1_baseline import EmotionCNN_V1 as EmotionCNN
    ...
    def switch_model(self):
        """Switch between V4 and V1 models."""
